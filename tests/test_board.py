...
def test_board():
    ...
    for x, y in start_positions:
        ...
        assert board.board[x][y] not in [const.PlayerColor.BLACK.value, const.PlayerColor.WHITE.value], (
    ...
    assert valid_moves == expected_moves, f"Expected valid moves {expected_moves}, but got {valid_moves}"
    ...
    new_expected_moves = [(1, 3), (2, 2), (3, 5), (5, 4)]  

    # Assertion for new board state
    assert new_valid_moves == new_expected_moves, f"Expected new valid moves {new_expected_moves}, but got {new_valid_moves}"

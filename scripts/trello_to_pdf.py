        ...
        for checklist in card.get('checklists', []):
            for item in checklist.get('checkItems', []):
                if "geschätzte" in item.get('name', '').lower():
                    task["Checklists"]["Geschätzte Bearbeitungszeit"] = item.get('name', '')
                elif "benötigte" in item.get('name', '').lower():
                    task["Checklists"]["Benötigte Bearbeitungszeit"] = item.get('name', '')
        
        tasks.append(task)
    
    # Sort tasks by LastActivity in descending order
    tasks.sort(key=lambda x: datetime.fromisoformat(x['LastActivity'].replace('Z', '+00:00')), reverse=True)

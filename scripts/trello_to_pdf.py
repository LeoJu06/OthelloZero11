                ...
                if "Geschätzte Bearbeitungszeit" in item_name:
                    try:
                        total_estimated_time += float(
                            "".join(filter(str.isdigit, item_name))
                        )
                    except ValueError:
                        pass
                    ...
                    try:
                        ...
                        pass
                items.append({"name": item_name, "state": item.get("state")})
            card_info["checklist"].append(
                {"name": checklist.get("name"), "items": items}
            )
...
def create_pdf_workflow(file_path, output_file):
    ...
    pdf.cell(
        200, 10, txt=f"Total Estimated Time: {total_estimated_time} minutes", ln=True
    )
    ...
    for step in workflow:
        ...
        for checklist in step["checklist"]:
            ...
            for item in checklist["items"]:
                ...
                pdf.set_text_color(0, 0, 0)
                pdf.multi_cell(
                    0, 10, txt=f"    - {item['name']} (Status: {item['state']})"
                )
        pdf.ln(5)

...
class Manager:
    ...
    def manage_workers(self, request_queue, response_queues, batch_size=20, timeout=0.1):
        while True:
            batch, worker_ids = [], []
            ...
            while len(batch) < batch_size:
                try:
                    ...
                    if len(batch) > 0 or (time.time() - start_time) >= timeout:
                        break

            if batch:
                policies, values = self.model.predict(np.array(batch))

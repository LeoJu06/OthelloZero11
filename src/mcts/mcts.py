...
class MCTS:
    ...
    def __init__(self, worker_id=None, root_node=None):
        ...
        if root_node is None:
            self.root = Node(prior=float("inf"))  # Initialize the root node with infinite prior.
        else:
    ...
    def _search_multicored(self, request_queue, response_queue):
        ...
        """
        for simulation in range(Hyperparameters.MCTS["num_simulations"]):
            print(f"Worker {self.worker_id}, simulation=>{simulation}")
            ...
            while node.is_expanded():
                action, node = node.select_child()  # Select the child node with the best UCB score.
            ...
            if node.is_terminal_state():  # Check if the game is over at this node.
                value = node.board.determine_winner()  # Determine the winner of the game.
            if value is None:  # If not terminal, request neural network predictions for the leaf node.
                leaf_node = search_path[-1]  # The current leaf node.
                request_queue.put((self.worker_id, leaf_node.board.board))  # Send the board state to the manager.

                # Wait for the manager's response.
                while True:
                    try:
                        response = response_queue.get_nowait()  # Non-blocking get from the response queue.
                        if response["worker_id"] == self.worker_id:  # Ensure the response matches this worker.
                            policy, value = response["policy"], response["value"]
                            node.expand(policy)  # Expand the node with the predicted policy.
                            break
    ...
    def _manage_multicored_search(self, request_queue, response_queues, batch_size=10):
        ...
        while True:
            ...
            while len(batch) < batch_size and not request_queue.empty():
                worker_id, state = request_queue.get()  # Retrieve the worker ID and board state.
            ...
            if batch:  # If there are states to process:
                policies, values = neural_network_evaluate(batch, model, device)  # Predict policies and values.

                # Send the results back to the respective workers.
                for worker_id, policy, value in zip(worker_ids, policies, values):
                    response_queues[worker_id].put({"worker_id": worker_id, "policy": policy, "value": value})

    def run_multicored(self):
        """
        Run the multi-core MCTS simulation by spawning manager and worker processes.
        """
        mp.set_start_method('spawn')  # Use 'spawn' to start new processes (recommended for compatibility).
        ...
        response_queues = [mp.Queue() for _ in range(num_workers)]  # Separate response queues for each worker.

        # Start the manager process.
        manager = mp.Process(target=self._manage_multicored_search, args=(request_queue, response_queues))
        manager.start()

        # Start worker processes.
        workers = []
        for i in range(num_workers):
            worker_mcts = MCTS(worker_id=i)  # Create a new MCTS instance for each worker.
            worker = mp.Process(target=worker_mcts._search_multicored, args=(request_queue, response_queues[i]))
